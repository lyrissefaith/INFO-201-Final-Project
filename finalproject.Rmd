...
# Changing Obesity data to wide form
longObesity <- short_obesity_data%>%
  pivot_wider(names_from = Year, values_from = Obesity)
longObesity <- longObesity[-c(39), ]

## Adding type
longObesity$type <- "obesity"
mergedpop$type <- "population"


mergedObesityPop=merge(longObesity, mergedpop,on='Point',all=T)
mergedObesityPop[is.na(mergedObesityPop)]=0
mergedObesityPop

mergedObesityPop <- mergedObesityPop[-c(1:5), ]

mergedObesityPop <- mergedObesityPop %>%
  relocate(type, .before = "2015")
```
