---
title: "Obesity Rates vs Population"
author: Sayako Huang, Sarah Lee, Faith Samson, Jamie Christiansen 
...
We are conducting a comprehensive analysis to examine the relationship between population size and obesity rates in the United States. Considering the societal, environmental, and systemic factors that influence health, our goal is to provide a multidimensional understanding that goes beyond individual behaviors. This study holds significance for both public health research and policy, contributing to a dynamic area of study.

Given our prior knowledge, examining the historical context to the present, the relationship between population size and well-being has transformed. Early societies grappled with challenges such as prevalent illnesses and viruses, leading to lower population figures. However, advancements in public health have substantially increased global population sizes, resulting in extended lifespans and higher fertility rates. Additionally, as per the World Health Organization, obesity numbers have tripled, dispelling the notion that only upper-income areas struggle with obesity. Lower and middle-class areas also face challenges in maintaining low obesity rates. Shockingly, more individuals are succumbing to overweight-related issues than preventable malnutrition. The WHO emphasizes that urban areas are grappling the most with obesity, a trend we aim to explore further based on region and population size in our data. Moreover, we know that America is viewed as more obese compared to foreign counterparts, and that some states have higher populations. We are interested to see if states with higher populations affect obesity numbers. We want to add the connection between obesity numbers and population numbers to put it all into perspective.

Our research delves into the impact of obesity on health, considering numerous diseases associated with it. By examining these numbers, focusing on hotspots, and raising awareness, we hope to contribute to maintaining high population numbers and minimizing obesity-related fatalities.

We are creating a dataset that combines the obesity percentage per state and the population data in that state to find patterns and trends between the size of the population and how obese they are in the USA. To examine correlations in more recent periods, we opted to assess the relationship between population size and obesity rates for each state in the United States by asking the following:

1. What is the highest and lowest obesity state in 2015? How did that progress to 2022?

2. Are there any correlations between a state’s population size and its obesity percentage? For example, do more populous states tend to have higher or lower obesity rates?
...
4. Are there any states where obesity has been increasing or decreasing at a faster rate? compare it to changes in population size?

Obesity is associated with various health issues, including chronic diseases. Identifying areas with higher obesity rates is crucial for targeting interventions by health professionals or public workers. Addressing our research questions is compelling as understanding the relationship between population sizes and obesity provides insights into societal impact. This knowledge can benefit healthcare systems, potentially boosting productivity over time. Additionally, recognizing patterns across states aids in tailoring interventions for specific regions, effectively addressing disparities. Studying the changes in obesity alongside population dynamics offers foresight into upcoming health concerns, guiding proactive measures.

# Our Data
We use the following datasets due to their comprehensive nature and credibility. It has all the data we want as it includes obesity rates for each state over the years and also has the population divided for each state over the years. 
...
For obesity data, the State of Childhood Obesity, sponsored by the Robert Wood Johnson Foundation, collected the data from the Behavioral Risk Factor Surveillance System (BRFSS) data, collected by the Centers for Disease Control and Prevention (aka. CDC).  
...
For the obesity data, we encountered some missing data, for example the obesity data for Puerto Rico, as for some data collection, Puerto Rico is not considered a state of the U.S., but a territory, and treated as different in some datasets.  
...
In our final dataset, _finalData_,  each row corresponds to a U.S. state, providing information on its population size or obesity rate, with data categorized by type, “Obesity” or “Population” for the years spanning from 2015 to 2022. Additionally with an additional column of growth/rate of change (percentage change) of obesity and population for years between 2015-2022. Our final summarizing data frame is called _summaryDf_. 

Following the processes of data cleaning and merging, we now have a cumulative total of 1,144 observations. Within the final dataset, there are a total of 104 rows, and 11 columns. 

The relevant variables are Area, type, years 2015-2022, and the growth/rate of change columns. They are coded as their own columns, with values in each year and are classified by the _type_ column, which indicates if the value is correlated to obesity or population data. 
...
**Variables:**
For this specific question, we utilized multiple variables and forms of our data, transforming it into wide and long forms, and selecting variables targeted to specific aspects of the question. We employed variables such as Year (specifically 2022 and 2015), Obesity, Population, and Area. We additionally created two-three additional columns, “type” and “ob_growth” and “pop_growth”, these two which can be found in the “growth_roc” column in our final df “finalData.” We found the states which have the highest and lowest rate of change in obesity and population separately using these variables and arranging them (desc()), to see if there was a correlation between high obesity rates of change and high population rates of change. 
We utilized the tools such as ggolot to plot the relationship and visualize the highest/lowest rates of change for obesity and population. 

**Data Processing:**
We initiate the process by transforming our finalMerge df to longer in order to add the classification of type to be able to group the data by “type” into a df called mergeLong. We then transform this df into a wider format to assign each year into its own column, into a df called mergeWide. Using this new df, we create separate df for obesity and population data in order to more easily manipulate the data to create our additional columns. We created a new df called mutateWide to create a df with just Area, years 2015 and 2022, and our new calculated ob_growth and pop_growth columns. To find the obesity and population growth by percentage growth, we take the value in 2022 and subtract the value in 2015 and divide by the 2015 value for each classification and each Area (state), removing NA values. Find the highest and lowest values of each obesity growth and population growth by arranging by arrange(desc()) function, and head() and tail() functions. 
For plotting, we create six total graphs to visualize the relationships between the growths and compare them. Utilizing ggplot we create a line graph with a trend line to visualize the highest/lowest obesity rate of change, as well as the highest/lowest population rate of change. To see if there is a relationship between the rates of change of obesity and population, we print the top and lowest 5 states for growth for both classifications. When we could not find the same names in each, we created a histogram to see the frequency and distribution of obesity and population growth. Using this, we can compare the other growth (obesity or population) of the highest/lowest growth states to see where their other growth lands on the distribution graph. The last two line graphs are comparing the obesity and population growths of the opposite highest and lowest states to see how they compare.

**Sampling:**
When answering all our questions, we chose to focus our sample selectionon the time frame spanning 2015 to 2022. This deliberate choice aimed to ensure our focus on pertinent and recent periods, enabling us to discuss results that remain relevant to our current context. Also, we figured our findings would be most beneficial to our target audience, the common public, if it was relevant. We hope our data will help those looking to find where they want to live and any other geographic decisions they might have.

**Technical Decisions:**
We overall did not need to remove values to attain our goals. 

**Statistical Methods:**
The only statistical methods we used were simple math and formulas. 
...
mergedpop <- left_join(short_pop_10_19, short_pop_20_22, by = join_by(Area))
```
...
finalMerge <- left_join(popLong, short_obesity_data, by = join_by(Area, Year)) 
```
...
Examining the graph, it's apparent that each state has sustained a relatively steady population size from 2015 to 2022. Notably, the states with the lowest obesity rates exhibit an inverse relationship in terms of population size: Colorado consistently holds the highest population, while the District of Columbia consistently has the lowest. Conversely, states with higher obesity rates are seen to fall between these two extremes with relatively low population sizes, with the exception of Louisiana, which has a much higher population size seen in 2015. Due to the state having the highest obesity only once, it can be hard to discern a pattern in the population of high obesity states.
...
finalData <- left_join(mergeWide, growthLong, by = join_by(Area, type))
...
mergeLong %>%
  ...
  geom_smooth(formula = y~x, method = "lm", se = FALSE, alpha = 0.3, size = 0.5)
```
...
mergeLong %>%
  ...
  geom_smooth(formula = y ~ x, method = "lm", se = FALSE, alpha = 0.8, size = 0.2)
```
...
mutateWide %>%
  ...
  geom_histogram(bins = 30, col = "gold1", fill = "green4") 
```
...
mutateWide %>%
  ...
  geom_histogram(bins = 30, col = "gold1", fill = "red3") 
```
...
mergeLong %>%
  ...
  geom_smooth(formula = y~x, method = "lm", se = FALSE, alpha = 0.5, size = 0.2)
```
...
ggplot(summaryDf, aes(ob_growth, pop_growth, col = Area)) +
  ...
  geom_smooth(formula = y~x, method = 'loess', mapping = aes(ob_growth, pop_growth, col = "green2"))
```
...
After reviewing the graph created by Louisiana and Colorado's Obesity trends, we came to the conclusion that the highest obesity percentage continued to trend upwards in obesity rates whereas the lowest continued to trend down. This information was important for us to note because it showed that patterns of obesity rates continued, no matter where on the spectrum they fell to begin with. We continued to learn about these trends in later questions.
